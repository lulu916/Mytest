...
if size(Tx_eff_Meff,1)*size(Tx_eff_Meff,2) > 1
    ...
    Q = 20*log10(q);
elseif Tx_eff_Meff == 1
    temp = Rx_eff(:).';
    %--------------------------------------------------------------%
    left = temp(temp<=0);
    right = temp(temp>0);
    avg_left = mean(left); avg_right = mean(right);
    D_temp_left = sum((abs(left - avg_left)).^2/(abs(avg_left))^2);
    D_temp_right = sum((abs(right - avg_right)).^2/(abs(avg_right))^2);
    D_temp = sqrt(D_temp_left + D_temp_right);
    q = sqrt(length([left right]))/D_temp;
    Q = 20*log10(q);
else
    Meff = Tx_eff_Meff;
